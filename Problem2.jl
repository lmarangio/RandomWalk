...
#Recall that this is a finite version of problem 2, in which:
#The ant cannot move outside a triangle and food is located in the diagonal segment of the triangle,
#passing trough (10,0), (0,10).
...
for i in 1: Int(1 + (M-m)/k)
    for j in 1 : Int(1 + (x_f-x_i)/k)
        ...
        if x +y < 10 
            if x == x_i && y != m 
                A[t, t-Int(1 + (x_f-x_i)/k)] = 1/3
                A[t, t+ Int(1 + (x_f-x_i)/k)] = 1/3
                A[t, t+1] = 1/3

            elseif  x == x_i && y == m
                A[t,t-Int(1 + (x_f-x_i)/k)] = 1/2     
                A[t, t+1] = 1/2
...
for i in 1: n
    for j in 1 :n 
        if A[i,j] == 1
            for k in i+1 :n
                for s in 1:n
                    if (A[k,s] == 1/4 || A[k,s] == 1/2 || A[k,s] == 1/3 ) && boo == 1
                        swaprow!(A, i, k)
                        swapcol!(A, i, k)
                        boo = 0
                    end
