...
# the boundary is defined by four functions f_u, f_d, f_l, f_r (up, down, left, right), and the food is outside the boundary.
# we also need to define the four intersection points.
function f_u(x)
    return (400 -x^2) 
end
...
#We manually, for now, circumscribe this graph in a square and discretize the square.
